...
// 공개 자산만 독립 배포 디렉터리에 복사한다. 저장소 전체를 복사하지 않는다.
...
const files = [
  '.nojekyll', 'index.html', 'css/style.css',
  'js/main.js', 'js/projects.js', 'js/contact.js',
  'images/profile.svg', 'images/loom.svg', 'images/deepquest.svg', 'images/tinypilot.svg'
];
const sensitive = /(?:gh[pousr]_[A-Za-z0-9]{20,}|github_pat_[A-Za-z0-9_]+|-----BEGIN .*PRIVATE KEY-----|010[- ]?\d{4}[- ]?\d{4}|\/Users\/hongyongjae\/|portfolio-demo\.tail2dac17\.ts\.net)/;
for (const file of files) {
  const data = fs.readFileSync(path.join(root, file));
  if (sensitive.test(data.toString('utf8'))) throw new Error(`공개 범위 재검토 필요: ${file}`);
...
console.log(JSON.stringify({ destination, files, privateFilesIncluded: false }, null, 2));
